...
from ts_networks import VORLAGEN_NAMEN
...
def _parse_vorlage(v):
    return v if v in VORLAGEN_NAMEN else VORLAGEN_NAMEN[0]
...
SETTING_SPECS = {
    "vorlage_select": SettingSpec("form", _parse_vorlage, VORLAGEN_NAMEN[0]),
...
def apply_preset(vorlage, material, lastfaktor, seed):
    st.session_state["vorlage_select"] = vorlage
...
def sync_query_params(vorlage, material, lastfaktor, seed):
    try:
        st.query_params["form"] = vorlage
